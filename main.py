...
class GeorotorApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        # TITRE DYNAMIQUE (Centré au-dessus de l'emplacement graphique)
        ...
        # On abaisse le top (0.80) pour laisser de la place aux légendes Matplotlib
    ...
    def charger_methode(self, methode_nom):
        ...
        configs = {
            ...
            "Trochoïde": [("Paramètres", [("N_lobes", "Dents Stator", 7, 1), ("R_prim", "Rayon primitif", 15.0, 0.5), ("R_traceur", "Rayon traceur", 1.0, 0.1)]), ("Résolution", [("nb_points", "Nb de points", 2000, 500)])],
    ...
    def generer_et_afficher(self):
        try:
            ...
            if methode == "Hybride":
                ...
                X, Y, _ = modeles.modele_trochoide(self.inputs['N_lobes'].get(), self.inputs['R_prim'].get(), self.inputs['R_traceur'].get(), self.inputs['nb_points'].get(), sm)
            ...
            # Profil
            self.ax_profil.plot(X, Y, color=couleur_stator, lw=2, label="Stator")
            ...
            # Application de la légende au-dessus du tracé (bbox_to_anchor permet de la sortir du cadre)
            ...
            if 'R_prim' in self.inputs: self.ax_schema.plot(self.inputs['R_prim'].get()*np.cos(th), self.inputs['R_prim'].get()*np.sin(th), 'b--', alpha=0.4, label="R_primitif")
            if 'R_ext' in self.inputs: self.ax_schema.plot(self.inputs['R_ext'].get()*np.cos(th), self.inputs['R_ext'].get()*np.sin(th), 'r--', alpha=0.4, label="R_extérieur")
            ...
            self.label_erreur.configure(text=f"Erreur de calcul")
